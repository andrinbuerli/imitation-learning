...
parser.add_argument(
    ...
    default=32,
    type=int,
    help="[Optional] Batch size to use for BC training.",
)
parser.add_argument(
    "--gail_timesteps",
    default=0,
...
def handle_onnx_export():
    ...
    if args.onnx_export_path is not None:
        ...
        onnx_model_path = str(path_onnx)
        export_model_as_onnx(learner, onnx_model_path, use_obs_array=True)
        import  onnx
        # Load exported model
        onnx_model  = onnx.load(onnx_model_path)

        print("Original IR version:", onnx_model .ir_version)

        # Downgrade for Godot's ONNX Runtime (max IR = 9)
        onnx_model .ir_version = 9

        # IMPORTANT: also force single-file ONNX (no .onnx.data)
        onnx.save_model(
            onnx_model ,
            onnx_model_path,
